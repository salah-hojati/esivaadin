...
@Route("admin/templates/Ftl")
@PermitAll
public class TemplateAdminViewFtl extends VerticalLayout {
    ...
    public TemplateAdminViewFtl(TemplateRepository templateRepository) {
        ...
        Button saveButton = new Button("Save", e -> {
            if (binder.validate().isOk()) {
                try {
                    ...
                    template.setContent(ToFtlConverter.convertToFtl(template.getContent(), new char[]{'#', '$'}));
                    template.setContent(new String(template.getContent().getBytes(StandardCharsets.UTF_8),StandardCharsets.UTF_8));
                    Template s = templateRepository.save(template);
